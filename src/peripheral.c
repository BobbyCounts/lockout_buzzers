...
// Used to update offset on new timing packets
static atomic_t sync_offset;
...
static ssize_t buzzer_data_read(struct bt_conn *conn, const struct bt_gatt_attr *attr, void *buf, uint16_t len, uint16_t offset)
{
	return 0;
}
...
BT_GATT_SERVICE_DEFINE(buzzer_service,
	...
	BT_GATT_CHARACTERISTIC(BT_UUID_BUZZER_ARM_CHAR,
		BT_GATT_CHRC_WRITE,
		BT_GATT_PERM_READ | BT_GATT_PERM_WRITE, buzzer_data_read, buzzer_arm_recieved, NULL),
	BT_GATT_CHARACTERISTIC(BT_UUID_BUZZER_PRESSED_CHAR,
...
static void connected(struct bt_conn *conn, uint8_t err)
{
	...
	// Clear buzzer state
	buzzer_device_reset();
	atomic_clear_bit(&sync_offset, 0);
...
static bool on_vs_evt(struct net_buf_simple *buf)
{
	...
	if(atomic_test_bit(&sync_offset, 0)){
		int diff = (evt->event_counter - last_timed_action.anchor_point_event_counter) * conn_interval_us;
		uint64_t current_central = last_timed_action.anchor_point_us_central_clock + diff;
		buzzer_update_offset(current_central, evt->anchor_point_us);

		// int64_t offset = current_central - evt->anchor_point_us;
		// uart_printf("Offset: %lld\n", offset);
